...
# مفتاحٌ قائم = هذا الخادم مُهيَّأ سلفًا. لا يُلمس — وتُكمَل المفاتيح الناقصة.
#
# فتحديثُ المثال في المستودع لا يصل `.env` القائم على الخادم: يُضاف متغيّرٌ
# جديد في نشرة، ويبقى الخادم على القديم صامتًا — يعمل بقيمةٍ افتراضية غير التي
# قُصدت، ولا شيء يقول ذلك. وهي أطول أنواع الفروق عمرًا لأن لا أحد يقارن ملفين.
if [ -f .env ] && grep -qE '^APP_KEY=base64:.+' .env; then
    ...
    while IFS= read -r line; do
        ...
        if [ -z "$value" ]; then
            # سرٌّ أو قيمةٌ لا افتراض لها: إضافتها فارغة تُخفيها في آخر الملف.
            manual="${manual} ${key}"
        else
            printf '%s\n' "$line" >> .env
            added=$((added + 1))
    ...
    [ "$added" -gt 0 ] && echo "  أُضيف ${added} متغيّرًا جديدًا من المثال بقيمه الافتراضية."
    [ -n "$manual" ] && echo "  ⚠️ ناقصٌ بلا قيمة افتراضية — اضبطه بيدك:${manual}"
    [ "$added" -eq 0 ] && [ -z "$manual" ] && echo "  ولا متغيّر ناقص."
